...
public class ProductServiceImpl implements ProductService {
    ...
    public ProductDTO toggleSelling(Long productId) {
        ...
        if (updateIsAcitve.getIsSelling() == false) {
            updateAllCart(productId);
        }
    ...
    @Transactional
    void updateAllCart(Long productId) {
    ...
    private void setProductFields(ProductDTO dto, Product existingProduct, Category category) {
        ...
        if (existingProduct.getOriginalPrice() != dto.getOriginalPrice() || existingProduct.getPercentDiscount() != dto.getPercentDiscount()) {
            updateCartItemPrice(dto);
        }
    }

    @Transactional
    void updateCartItemPrice(ProductDTO dto) {
        BigDecimal price = calculateDiscountedPrice(dto.getOriginalPrice(), dto.getPercentDiscount());
        cartItemRepository.updatePriceAndSubtotal(price, dto.getId());
    }
